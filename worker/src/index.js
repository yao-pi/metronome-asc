 ...
 *
 * Endpoints:
 *   POST /auth      { accessToken }              -> { user, sessionToken, expiresAt }
...
async function handleSession(request, env) {
  ...
  const claims = await readSession(env, token);
  if (!claims) {
...
export default {
  async fetch(request, env) {
    ...
    if (pathname === "/health") {
      return json({ ok: true, configured: Boolean(env.SESSION_SECRET) }, 200, request, env);
    }
    ...
    if (pathname === "/auth" && request.method === "POST") {
      ...
      return handleAuth(request, env, payload);
    }
